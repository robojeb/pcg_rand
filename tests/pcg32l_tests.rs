...
fn Pcg32L_seq_diff() {
    for _ in 0..NUM_TESTS {
        ...
        let seed : u64 = thread_rng().gen();
        let seq : u64 = thread_rng().gen();
        let mut ra : Pcg32L = Pcg32L::from_seed(PcgSeeder::seed_with_stream(seed as u128, seq as u128));
        let mut rb : Pcg32L = Pcg32L::from_seed(PcgSeeder::seed_with_stream(seed as u128, (seq + 2) as u128));
...
fn Pcg32L_seed_diff() {
    for _ in 0..NUM_TESTS {
        ...
        let seq : u64 = thread_rng().gen();
        let mut ra : Pcg32L = Pcg32L::from_seed(PcgSeeder::seed_with_stream(seed as u128, seq as u128));
        let mut rb : Pcg32L = Pcg32L::from_seed(PcgSeeder::seed_with_stream((seed + 1) as u128, seq as u128));
        assert!(ra.sample_iter(&Alphanumeric).take(100).collect::<Vec<_>>() !=
                rb.sample_iter(&Alphanumeric).take(100).collect::<Vec<_>>());
    }
}
